...
const mintNftAddress = '0x33Ed3336Cb2Bf0CdDA5EAE7C336bdfC649F2a77d';
const mintNftAbi = require('../build/contracts/MintNft.json').abi;

const stakeAddress = '0x7Ce165a79B043fd77Fe35B2DDbd93DFd30644E7E'
// const stakeAbi = require('../build/contracts/Stake.json').abi;

const getNftsAddress = '0xD56d40B9B24388a266723C4A2d5e30e200d1e2a0'
const getNftsAbi = require('../build/contracts/GetMintNft.json').abi;
...
const run = async () => {
    ...
    const mintNftContract = new web3.eth.Contract(mintNftAbi, mintNftAddress);
    // const stakeContract = new web3.eth.Contract(stakeAbi, stakeAddress);
    const getNftsContract = new web3.eth.Contract(getNftsAbi, getNftsAddress);
    
    // const setStakeAddress = await mintNftContract.methods.setStakeAddress(stakeAddress).send({from : address})
    // console.log(setStakeAddress);

    const getS = await mintNftContract.methods.stakeAddress().call()
    console.log(getS);

    // const addNfts = await mintNftContract.methods
    ...
    // const getNFTS = await getNftsContract.methods.getNFTS().call()
    // console.log(getNFTS);
    ...
    // const addOneNft =await mintNftContract.methods.addOneNft(9,1,20000,"https://ipfs.io/ipfs/QmcvkFEfq2E1UZ16MKG6yyaZyTD7zc2xyrfcftYnjJZF7D").send({from : address})
    // console.log(addOneNft);
}
